    ...
    for line in fopen.readlines():
        ...
        if now_data == "show system alarms":
            ...
            if "No alarms" in input_line:
                ...
                "No alarms" not in input_line
                and "{master" not in input_line
                and hostname not in input_line
            ):
            ...
            if "Chassis" in input_line and "Virtual Chassis" not in input_line:
                ...
                "FPC" in input_line
                and "FPC CPU" not in input_line
                and "MX" not in str(hostname)
            ):
                if str(dev_sn) in str(
                    re.findall(r"FPC\s..\s*.{1,6}\s*.{1,11}\s*(.*)", input_line)
                ):
                    dev_sn = str(
                        re.findall(r"FPC\s..\s*.{1,6}\s*.{1,11}\s*(.*)", input_line)
                    )
            ...
            if "Model" in input_line:
                ...
                "3200" in str(hostname) or "4248" in str(hostname)
            ) and "JUNOS Base OS boot" in input_line:
                dev_ver = (
                    str(dev_ver)
                    + " "
                    + str(re.findall(r"JUNOS Base OS boot\s(.*)", input_line))
                    + "\n"
                )
                ...
                dev_ver = (
                    str(dev_ver)
                    + " "
                    + str(re.findall(r"Junos:\s(.*)", input_line))
                    + "\n"
                )
            ...
            if tmp_fpc == b"":
                ...
                if "System booted:" in input_line:
                    uptime_list[1] = (
                        str(now_fpc) + " " + str(re.findall(r"ted:\s*(.*)", input_line))
                    )
                ...
                if "Last configured:" in input_line:
                    uptime_list[3] = (
                        str(now_fpc) + " " + str(re.findall(r"red:\s*(.*)", input_line))
                    )
            else:
                if "Current time:" in input_line:
                    uptime_list[0] = (
            ...
            if tmp_slot == b"":
                if "Current state" in input_line:
                    re_list[0] = (
                        str(now_slot)
                        + " "
                        + str(re.findall(r"state\s*(.*)", input_line))
                    )
                if "CPU temperature" in input_line:
                    re_list[1] = (
                        str(now_slot)
                        + " "
                        + str(re.findall(r"ture\s*(.*?)\sdeg", input_line))
                        + " °C"
                    )
                ...
                if "Memory utilization" in input_line:
                    re_list[3] = (
                        str(now_slot)
                        + " "
                        + str(re.findall(r"zation\s*(.*)\sper", input_line))
                        + "%"
                    )
